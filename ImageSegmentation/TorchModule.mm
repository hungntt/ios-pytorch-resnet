...
- (unsigned char*)segmentImage:(void *)imageBuffer withWidth:(int)width withHeight:(int)height {
    try {
        ...
        const int CABLE = 1;
        const int TOWER_LATTICE = 2;
        const int TOWER_TUCOHY = 3;
        const int TOWER_WOODEN = 4;
        ...
        for (int j = 0; j < width; j++) {
            for (int k = 0; k < height; k++) {
                ...
                if (maxi == CABLE) buffer[n] = 255;
                else if (maxi == TOWER_LATTICE) buffer[n+1] = 255;
                else if (maxi == TOWER_TUCOHY) buffer[n+2] = 255;
                else if (maxi == TOWER_WOODEN) buffer[n+3] = 255;
            }
        }
